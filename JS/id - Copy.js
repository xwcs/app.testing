...
var CsBridge = (function(ind){
    ...
    */
    ...
    var ret = {
        ...
        addEventHandler : function(evtSource, evtKind, csHandlerId){
            /*
            if(_handlers.hasOwnProperty(csHandlerId)){
                if(_handlers[csHandlerId].sender == evtSource && _handlers[csHandlerId].kind ==  evtKind)
                    return true; // we have already registered bridge
            }else{
                return "csHandlerId used!";
            }
            // register new bridge
            _handlers[csHandlerId] = {
                sender: evtSource,
                kind: evtKind
            };
            */
            evtSource.addEventListener(evtKind, function(evt){
                alert('Event here');
                __eventHandler(csHandlerId, evt);    
            });    
        },
    ...
    var __eventHandler = function(csHandlerId, evt){
        _tube.write(JSON.stringify(
            {
                csHandlerId : csHandlerId,
                evt : {
                    bubbles : evt.bubbles,
        ...
        if(reply.evt.defaultPrevented) evt.preventDefault();
        if(reply.evt.propagationStopped) evt.stopPropagation();
    }
...
//app.menuActions[4].addEventListener('beforeInvoke', myEventHandler);